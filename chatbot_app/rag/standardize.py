...
"""
	...
	""" 
def standardize_vietnamese_word(word):
...
def standardize_vietnamese_sentence(sentence):
    """
    ...
    for index, word in enumerate(words):
        ...
        # print(cw)
        if len(cw) == 3:
            cw[1] = standardize_vietnamese_word(cw[1])
        words[index] = ''.join(cw)
    return ' '.join(words)
...
def preprocess_text(text):
	...
	text = standardize_vietnamese_sentence(text)
...
if __name__ == "__main__":
    file = open("D:/Desktop/output_chunks_1.txt", "r", encoding="utf-8")
    data = file.read()
